...
export default class NewProjectView {
      ...
      <div class="inset-panel">
        ...
        <div class="panel-body padded">
          ...
          <div class="footer-container">
            <div class='loading'>
              <span class='loading-message'><span id="message">${this.message}</span></span>
            </div>
  ...
  createProject() {
    ...
    const openFiles = projectInfo.openFiles ? projectInfo.openFiles.map(f => path.join(this.projectPath, f)) : [];
    this.setMessage('Creating project...');
    setTimeout(() => {
      exec(`git clone "${repoPath}" "${this.projectPath}"`, (err, stdout, stderr) => {
        if(err) {
          const errorMessage = stderr.replace(/^fatal: /, '').trim();
          this.setMessage(errorMessage, 'status-removed');
          return;
        }

        fs.removeSync(path.join(this.projectPath, '.git'));
        atom.open({ pathsToOpen: [this.projectPath].concat(openFiles) });
        this.close();
      });
    }, 0);
  }
  ...
  render() {
    ...
    this.editor.onDidChange(() => this.projectPath = this.editor.getText());
    this.attachEvents();
  }
}
