...
const runMenu = {
  label: 'Run',
  submenu: [
    { label: 'Run Current Python File', command: 'north-editor:run-current-python-file' },
    { label: 'Run main.py', command: 'north-editor:run-main-python-file' },
    { label: 'Open Python Console', command: 'north-editor:ipython-terminal' },
    { label: 'Install Python Requirements', command: 'north-editor:install-requirements' }
  ]
}
...
export default {
  ...
  isNewWindow() {
    ...
    return atom.project.getPaths().length === 0 && editors.length === 1 && editors[0].getTitle() === 'untitled' && editors[0].getText() === '';
  },
  ...
  ipythonTerminal() {
    const pythonShell = atom.config.get('north-editor-package.pythonShellPath');
    this.terminal.run([pythonShell, 'exit']);
  },

  installRequirements() {
    const requirementsPath = path.join(this.findCurrentProjectPath(), 'requirements.txt');

    if(!fs.existsSync(requirementsPath)) {
      atom.notifications.addError(`Cannot find "requiments.txt" in project root`);
      return;
    }

    atom.notifications.addInfo('Installing Python requirements...');
  ...
  openSimulator() {
    ...
    if(!fs.existsSync(simulatorPath)) {
      atom.notifications.addError('N9 Simulator not found');
      return;
    }

    atom.notifications.addInfo('Opening N9 Simulator...');
  ...
  runCurrentPythonFile() {
    ...
    if(fileType.toLowerCase() !== '.py') {
      atom.notifications.addError(`"${path.basename(filePath)}" is not a Python file`);
      return;
    }
  ...
  runMainPythonFile() {
    ...
    if(!fs.existsSync(mainPath)) {
      atom.notifications.addError(`No "main.py" file found`);
      return;
    }
